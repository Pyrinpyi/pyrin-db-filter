...
class PyipadThread(object):
    def __init__(self, pyipad_host, pyipad_port, async_thread=True):

        self.pyipad_host = pyipad_host
        self.pyipad_port = pyipad_port

        if async_thread:
            self.channel = grpc.aio.insecure_channel(f'{pyipad_host}:{pyipad_port}',
            ...
            self.channel = grpc.insecure_channel(f'{pyipad_host}:{pyipad_port}',
                                                 compression=grpc.Compression.Gzip,
                                                 options=[
                                                     ('grpc.max_send_message_length', MAX_MESSAGE_LENGTH),
                                                     ('grpc.max_receive_message_length', MAX_MESSAGE_LENGTH),
